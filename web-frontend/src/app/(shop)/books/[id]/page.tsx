...
// Component Render Comment (Giữ nguyên)
function CommentTree({
  comment,
  depth = 0,
  replyOpen,
...
export default function BookDetailPage() {
  ...
  // ... (Các hàm updateArrows và scrollByStep giữ nguyên để code gọn hơn)
  function updateArrowsLike() {
    const el = likeRef.current;
    if (!el) return;
    const { scrollLeft, scrollWidth, clientWidth } = el;
  ...
  // Review & Comment Logic (Giữ nguyên)
  const [newReview, setNewReview] = useState("");
  ...
  function handleBuyNow() {
    ...
    // KHÔNG gọi handleAddToCart()
    // Chuyển hướng trực tiếp với thông tin "Mua ngay"
