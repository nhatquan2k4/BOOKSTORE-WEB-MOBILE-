...
//               let isSelected = true;
//               if (buyNowId) {
//                  isSelected = item.bookId === buyNowId;
//               }
...
//   // =====================================================================
//   // 4. CALCULATIONS (ĐÃ SỬA LOGIC SHIP)
//   // =====================================================================
...
//   // --- SỬA ĐỔI: Luôn miễn phí vận chuyển để khớp với Backend ---
//   const shippingFee = 0;
...
//                 <div
//                   key={item.id}
//                   className="bg-white p-4 rounded-lg shadow-sm flex gap-4 transition-all hover:shadow-md"
//                 >
//                   <div className="pt-2">
//                     <input
//                       type="checkbox"
...
//                         className="font-semibold text-gray-900 text-lg hover:text-blue-600 line-clamp-2"
//                       >
//                         {item.title}
//                       </Link>
...
//                             -
//                           </button>
//                           <span className="px-2 w-8 text-center font-medium">
//                             {item.quantity}
//                           </span>
//                           <button
//                             className="px-3 py-1 hover:bg-gray-100 disabled:opacity-50"
...
//                     disabled={applyingCoupon || !voucherCode}
//                   >
//                     {applyingCoupon ? "..." : "Add"}
//                   </Button>
//                 </div>
//                 {voucherError && (
//                   <p className="text-xs text-red-600 mb-2">{voucherError}</p>
//                 )}
...
//                     <span>
//                       Đã dùng mã: <strong>{appliedVoucher.code}</strong>
//                     </span>
//                     <button
...
//                             {couponService.formatCouponDescription(c)}
//                           </div>
//                         </div>
//                       ))}
//                     </div>
...
//                 <Button
//                   className="w-full mt-4 py-3 font-bold bg-blue-600 text-white hover:bg-blue-700"
...
export default function CartPage() {
  ...
  useEffect(() => {
    const fetchCartAndSync = async () => {
      try {
        ...
        console.log('🛒 CART PAGE: Loading cart from backend...');
        ...
        console.log(`📦 Cart loaded: ${rawItems.length} items`);
        if (rawItems.length > 0) {
          console.log('Cart items:', rawItems.map((i: any) => ({ bookId: i.bookId, title: i.bookTitle, qty: i.quantity })));
        }
          ...
          rawItems.map(async (item: any) => {
            try {
              ...
              return {
                id: item.id,
                bookId: item.bookId,
                title: item.bookTitle || "Sản phẩm lỗi",
                author: "",
                cover: "/image/anh.png",
                price: item.unitPrice || 0,
                quantity: item.quantity,
    ...
    const loadCoupons = async () => {
         try {
             const res: any = await couponService.getPublicCoupons();
             const list = Array.isArray(res) ? res : res.items || [];
             setPublicCoupons(list.filter((c: any) => !couponService.isExpired(c)));
         } catch(e) {}
    }
    loadCoupons();

    // Listen for cart updates (e.g., after checkout removes items)
  ...
  const handleUpdateQuantity = async (itemId: string, newQty: number) => {
    ...
    try { await cartService.updateCartItemQuantity(item.bookId, newQty); }
    catch { /* Revert if fail */ }
  };

  const handleRemoveItem = async (itemId: string) => {
    const item = cartItems.find((i) => i.id === itemId);
    if (!item || !confirm("Xóa sách này?")) return;
    try {
        await cartService.removeCartItem(item.bookId);
        setCartItems(prev => prev.filter(i => i.id !== itemId));
        
        // Dispatch event để cập nhật cart count ở header
        if (typeof window !== 'undefined') {
  ...
  const handleApplyVoucher = async () => {
    ...
    try {
        const res = await couponService.validateCoupon(voucherCode, subtotal);
        if(res.success && res.coupon) { setAppliedVoucher(res.coupon); setVoucherError(""); }
        else { setVoucherError(res.message || "Mã lỗi"); }
    } catch { setVoucherError("Lỗi kiểm tra"); }
  ...
  const handleCheckout = () => {
    ...
    // Lấy ID của các sách ĐANG ĐƯỢC CHỌN (selected: true)
    const selectedBookIds = selectedItems.map(i => i.bookId).join(',');
    
    // Chuyển sang trang Checkout, truyền danh sách ID qua URL
    router.push(`/checkout?items=${selectedBookIds}`);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4">
        <h1 className="text-3xl font-bold mb-6">Giỏ hàng ({cartItems.length})</h1>
        
        {cartItems.length === 0 ? (
           <EmptyState title="Giỏ hàng trống" description="Chưa có sách nào" action={{ label: "Mua sắm ngay", onClick: () => router.push('/books') }} />
        ) : (
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              {/* ITEM LIST */}
              <div className="lg:col-span-2 space-y-4">
                        ...
                        <div className="flex-1">
                           ...
                           <div className="flex items-center gap-4 mt-2">
                              <div className="border rounded flex">
                                 <button className="px-3" onClick={() => handleUpdateQuantity(item.id, item.quantity - 1)}>-</button>
                                 <span className="px-2 py-1">{item.quantity}</span>
                                 <button className="px-3" onClick={() => handleUpdateQuantity(item.id, item.quantity + 1)}>+</button>
                              </div>
                              <button onClick={() => handleRemoveItem(item.id)} className="text-red-500 text-sm">Xóa</button>
                           </div>
                        </div>
                    </div>
                 ))}
              ...
              <div className="lg:col-span-1">
                 <div className="bg-white p-6 rounded shadow sticky top-4">
                    ...
                    <div className="flex gap-2 mb-2">
                       <Input value={voucherCode} onChange={e => setVoucherCode(e.target.value.toUpperCase())} placeholder="Mã Voucher" />
                       <Button size="sm" onClick={handleApplyVoucher} disabled={applyingCoupon}>Áp dụng</Button>
                    </div>
                    ...
                    <div className="border-t pt-4 space-y-2">
                       ...
                       <div className="flex justify-between text-green-600"><span>Giảm giá</span><span>-{formatPrice(discountAmount)}</span></div>
                       <div className="flex justify-between font-bold text-xl text-red-600 pt-2 border-t">
                          <span>Tổng cộng</span><span>{formatPrice(finalTotal)}</span>
                       </div>
                    </div>

                    <Button className="w-full mt-4 bg-blue-600 text-white font-bold py-3" 
                            onClick={handleCheckout} 
                            disabled={selectedItems.length === 0}>
                        Thanh toán ({selectedItems.length})
                    </Button>
